...
class Policy:
    ...
    def add_policy(
        ...
        if (
            isinstance(policy_info, (PacketPolicy, HttpPolicy))  # | 대신, 튜플로 처리
    ...
    def delete_policy(self, policy_type: str, policy_name: str):
        ...
        if policy_name not in self.policies[policy_type]:
            raise ValueError(
                f"Policy '{policy_name}' does not exist in '{policy_type}' policies."
            )

        del self.policies[policy_type][policy_name]
        self.save()  # 변경된 내용을 저장
        print(f"Policy '{policy_name}' successfully deleted from '{policy_type}'.")
